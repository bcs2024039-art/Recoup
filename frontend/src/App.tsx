      ...
      {onExit && (
        <nav className="h-16 px-6 lg:px-12 flex items-center justify-between border-b border-stone-800 bg-[#161616]">
           <button onClick={async () => {
             ...
             onExit();
           }} className="text-sm font-medium text-stone-400 hover:text-stone-300 transition flex items-center gap-2">
             &larr; Sign out
           </button>
...
function AuthPage({ onLogin }: { onLogin: () => void }) {
  const [email, setEmail] = useState("admin@recoup.com");
      ...
      if (isSignUp) {
        ...
        try {
          ...
          if (signInErr.code === 'auth/invalid-credential' || signInErr.code === 'auth/user-not-found') {
             // For demo purposes, auto sign up if account doesn't exist
             await createUserWithEmailAndPassword(auth, email, password);
          } else {
             throw signInErr;
          }
    ...
    >
      <div className="bg-[#1a1a1a] p-10 rounded-2xl border border-stone-800 shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : (isSignUp ? "Sign Up" : "Sign in to Recoup")}
          </button>
        </form>

        <div className="mt-6 flex items-center justify-between">
          <hr className="w-full border-stone-800" />
          <span className="px-3 text-xs text-stone-500 uppercase">Or</span>
          <hr className="w-full border-stone-800" />
        </div>
        ...
        <button 
          ...
          disabled={loading}
          className="mt-6 w-full flex items-center justify-center gap-2 bg-[#111111] border border-stone-700 text-stone-300 text-sm font-medium py-3 rounded-lg hover:bg-stone-800 transition disabled:opacity-50"
        >
